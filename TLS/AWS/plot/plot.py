...
legend_labels = ['Paris', 'Frankfurt', 'N. California', 'NULL'] 
...
def plot_histogram(df, ax, title):
    ...
    for i, (col, color) in enumerate(zip(cols, colors)):
        ...
        for j in range(n_algs):
            ...
            ax.bar(
                ...
                df[col].iloc[j],
                bar_width,
                label=legend_labels[i] if (j == 0 and i < 3) else "",
                color=color,
                alpha=alpha,
            )
    
    # Customize the plot
    ax.set_ylabel('TLS Handshakes')
    ax.set_title(title)
    ...
    y_max = 10**4 * 1.5  # a bit above the top
    dash_height = 0.08 * 10**4  # length of the vertical dash
    text_offset = 0.18 * 10**4  # vertical offset for the text

    # left group
    x_start1 = positions[0] - 0.3
    x_end1 = positions[idx_cat_sep[subplot_idx]] + 0.3
    ax.plot([x_start1, x_end1], [y_max, y_max], color='grey', lw=1.5, clip_on=False)
    ax.plot([x_start1, x_start1], [y_max - dash_height, y_max + dash_height], color='grey', lw=1.5, clip_on=False)
    ax.plot([x_end1, x_end1], [y_max - dash_height, y_max + dash_height], color='grey', lw=1.5, clip_on=False)
    ...
    ax.plot([x_start2, x_end2], [y_max, y_max], color='grey', lw=1.5, clip_on=False)
    ax.plot([x_start2, x_start2], [y_max - dash_height, y_max + dash_height], color='grey', lw=1.5, clip_on=False)
    ax.plot([x_end2, x_end2], [y_max - dash_height, y_max + dash_height], color='grey', lw=1.5, clip_on=False)
